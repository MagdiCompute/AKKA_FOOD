...
jest.mock("firebase-functions/v2/https", () => ({
  onCall: (handler: (request: unknown) => Promise<unknown>) => {
...
import "../admin/adminUpdateOrderStatus";

// ── Helpers ───────────────────────────────────────────────────────────────────

function makeRequest(data: Record<string, unknown>, uid = "admin-uid") {
  return {
    auth: { uid, token: {} },
    data,
  };
...
function makeOrderSnapshot(exists: boolean, data?: Record<string, unknown>) {
  return {
    exists,
    data: () => data,
  };
...
describe("adminUpdateOrderStatus", () => {
  ...
  it("calls verifyAdmin with the request auth context", async () => {
    mockGet.mockResolvedValueOnce(
      makeOrderSnapshot(true, { status: "pending", uid: "user-1" })
    );
  ...
  it("rejects unauthenticated callers (verifyAdmin throws)", async () => {
    mockVerifyAdmin.mockRejectedValueOnce(
      Object.assign(new Error("The function must be called while authenticated."), {
        code: "unauthenticated",
      })
    );

    await expect(
      capturedHandler({ auth: undefined, data: { orderId: "order-1", status: "confirmed" } })
    ).rejects.toMatchObject({ code: "unauthenticated" });
  });

  it("rejects non-admin callers (verifyAdmin throws permission-denied)", async () => {
    mockVerifyAdmin.mockRejectedValueOnce(
      Object.assign(new Error("Admins only"), { code: "permission-denied" })
    );

    await expect(
      capturedHandler(makeRequest({ orderId: "order-1", status: "confirmed" }))
  ...
  it("throws invalid-argument when orderId is missing", async () => {
    await expect(
      capturedHandler(makeRequest({ status: "confirmed" }))
    ).rejects.toMatchObject({ code: "invalid-argument" });
  });

  it("throws invalid-argument when orderId is not a string", async () => {
    await expect(
      capturedHandler(makeRequest({ orderId: 123, status: "confirmed" }))
    ).rejects.toMatchObject({ code: "invalid-argument" });
  });

  it("throws invalid-argument when status is missing", async () => {
    await expect(
      capturedHandler(makeRequest({ orderId: "order-1" }))
    ).rejects.toMatchObject({ code: "invalid-argument" });
  });

  it("throws invalid-argument when status is not a recognised value", async () => {
    await expect(
      capturedHandler(makeRequest({ orderId: "order-1", status: "flying" }))
    ).rejects.toMatchObject({ code: "invalid-argument" });
  });

  it("throws invalid-argument for statuses not in the valid set", async () => {
    await expect(
      capturedHandler(makeRequest({ orderId: "order-1", status: "ready_for_pickup" }))
    ).rejects.toMatchObject({ code: "invalid-argument" });

    await expect(
      capturedHandler(makeRequest({ orderId: "order-1", status: "cancelled" }))
    ).rejects.toMatchObject({ code: "invalid-argument" });
  });
  ...
  it("throws invalid-argument when status is out_for_delivery and etaMinutes is missing", async () => {
    await expect(
      capturedHandler(
        makeRequest({ orderId: "order-1", status: "out_for_delivery" })
      )
    ).rejects.toMatchObject({ code: "invalid-argument" });
  });
  ...
  it("throws invalid-argument when etaMinutes is not a number for out_for_delivery", async () => {
    await expect(
      capturedHandler(
        makeRequest({ orderId: "order-1", status: "out_for_delivery", etaMinutes: "thirty" })
      )
    ).rejects.toMatchObject({ code: "invalid-argument" });
  });
  ...
  it("throws not-found when order does not exist", async () => {
    ...
    await expect(
      capturedHandler(makeRequest({ orderId: "ghost-order", status: "confirmed" }))
  ...
  describe("valid transitions", () => {
    it("allows transition pending → confirmed", async () => {
      ...
      const result = await capturedHandler(
        makeRequest({ orderId: "order-1", status: "confirmed" })
      );
    ...
    it("allows transition confirmed → preparing", async () => {
      mockGet.mockResolvedValueOnce(
        makeOrderSnapshot(true, { status: "confirmed", uid: "user-1" })
      );
      const result = await capturedHandler(
        makeRequest({ orderId: "order-1", status: "preparing" })
      );
      expect(result).toEqual({ success: true });
      expect(mockUpdate).toHaveBeenCalledTimes(1);
    });
    ...
    it("allows transition preparing → out_for_delivery (with etaMinutes)", async () => {
      mockGet.mockResolvedValueOnce(
        makeOrderSnapshot(true, { status: "preparing", uid: "user-1" })
      );
      const result = await capturedHandler(
        makeRequest({ orderId: "order-1", status: "out_for_delivery", etaMinutes: 25 })
      );
      expect(result).toEqual({ success: true });
      expect(mockUpdate).toHaveBeenCalledTimes(1);
    });
    ...
    it("allows transition out_for_delivery → delivered", async () => {
      mockGet.mockResolvedValueOnce(
        makeOrderSnapshot(true, { status: "out_for_delivery", uid: "user-1" })
      );
      const result = await capturedHandler(
        makeRequest({ orderId: "order-1", status: "delivered" })
      );
      expect(result).toEqual({ success: true });
      expect(mockUpdate).toHaveBeenCalledTimes(1);
    });
    ...
    it("allows transition out_for_delivery → failed", async () => {
      ...
      const result = await capturedHandler(
        makeRequest({ orderId: "order-1", status: "failed" })
      );
      expect(result).toEqual({ success: true });
      expect(mockUpdate).toHaveBeenCalledTimes(1);
    });
  ...
  describe("invalid transitions", () => {
    ...
    it.each(invalidTransitions)(
      "throws failed-precondition for %s → %s",
      async (from, to) => {
        ...
        if (to === "out_for_delivery") requestData["etaMinutes"] = 30;

        mockGet.mockResolvedValueOnce(
          makeOrderSnapshot(true, { status: from, uid: "user-1" })
        );

        await expect(
          capturedHandler(makeRequest(requestData))
  ...
  it("throws failed-precondition when current status is unrecognised", async () => {
    mockGet.mockResolvedValueOnce(
      makeOrderSnapshot(true, { status: "unknown_status", uid: "user-1" })
    );
  ...
  it("writes the new status and updatedAt to Firestore", async () => {
    ...
    await capturedHandler(
      makeRequest({ orderId: "order-42", status: "confirmed" })
    );
  ...
  it("writes etaMinutes to Firestore when transitioning to out_for_delivery", async () => {
    ...
    await capturedHandler(
      makeRequest({ orderId: "order-42", status: "out_for_delivery", etaMinutes: 15 })
    );
  ...
  it("sends a push notification to the customer on successful status update", async () => {
    mockGet.mockResolvedValueOnce(
      makeOrderSnapshot(true, { status: "confirmed", uid: "customer-uid" })
    );
  ...
  it("sends etaMinutes in the notification for out_for_delivery", async () => {
    mockGet.mockResolvedValueOnce(
      makeOrderSnapshot(true, { status: "preparing", uid: "customer-uid" })
    );
  ...
  it("does not send a notification when the order has no uid", async () => {
    mockGet.mockResolvedValueOnce(
      makeOrderSnapshot(true, { status: "confirmed" }) // no uid field
    );
