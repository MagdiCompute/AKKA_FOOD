...
jest.mock("firebase-functions/v2/https", () => ({
  onCall: (optionsOrHandler: unknown, handler?: (request: unknown) => Promise<unknown>) => {
    ...
    if (typeof optionsOrHandler === "function") {
      capturedHandler = optionsOrHandler as (request: unknown) => Promise<unknown>;
    } else if (handler) {
...
function makeRequest(data: Record<string, unknown>, auth?: { uid: string }) {
  return {
    auth: auth || null,
    data,
  };
...
describe("initiatePayment", () => {
  ...
  describe("authentication", () => {
    it("throws unauthenticated when no auth token is provided", async () => {
      await expect(
        capturedHandler(makeRequest({ amount: 2000, phoneNumber: "70123456" }))
      ).rejects.toMatchObject({ code: "unauthenticated" });
    });

    it("throws unauthenticated when auth is null", async () => {
      await expect(
        capturedHandler({ auth: null, data: { amount: 2000, phoneNumber: "70123456" } })
      ).rejects.toMatchObject({ code: "unauthenticated" });
    });
  });
  ...
  describe("input validation", () => {
    it("throws invalid-argument when amount is missing", async () => {
      await expect(
        capturedHandler(makeRequest({ phoneNumber: "70123456" }, { uid: "user-1" }))
      ).rejects.toMatchObject({ code: "invalid-argument" });
    });

    it("throws invalid-argument when amount is zero", async () => {
      await expect(
        capturedHandler(makeRequest({ amount: 0, phoneNumber: "70123456" }, { uid: "user-1" }))
      ).rejects.toMatchObject({ code: "invalid-argument" });
    });

    it("throws invalid-argument when amount is negative", async () => {
      await expect(
        capturedHandler(makeRequest({ amount: -500, phoneNumber: "70123456" }, { uid: "user-1" }))
      ).rejects.toMatchObject({ code: "invalid-argument" });
    });

    it("throws invalid-argument when amount is not an integer", async () => {
      await expect(
        capturedHandler(makeRequest({ amount: 1500.5, phoneNumber: "70123456" }, { uid: "user-1" }))
      ).rejects.toMatchObject({ code: "invalid-argument" });
    });

    it("throws invalid-argument when phoneNumber is missing", async () => {
      await expect(
        capturedHandler(makeRequest({ amount: 2000 }, { uid: "user-1" }))
      ).rejects.toMatchObject({ code: "invalid-argument" });
    });

    it("throws invalid-argument when phoneNumber is invalid format", async () => {
      await expect(
        capturedHandler(makeRequest({ amount: 2000, phoneNumber: "123" }, { uid: "user-1" }))
      ).rejects.toMatchObject({ code: "invalid-argument" });
    });
    ...
    it("accepts a valid 8-digit Mali phone number", async () => {
      ...
      const result = await capturedHandler(
        makeRequest({ amount: 2000, phoneNumber: "70123456" }, { uid: "user-1" })
      );
    ...
    it("accepts a phone number with +223 prefix", async () => {
      ...
      const result = await capturedHandler(
        makeRequest({ amount: 2000, phoneNumber: "+22370123456" }, { uid: "user-1" })
      );
  ...
  describe("transaction creation", () => {
    it("creates a transaction document in Firestore with status pending", async () => {
      ...
      await capturedHandler(
        makeRequest({ amount: 3000, phoneNumber: "70123456" }, { uid: "user-1" })
      );
    ...
    it("includes orderId when provided", async () => {
      ...
      await capturedHandler(
        makeRequest(
          { amount: 3000, phoneNumber: "70123456", orderId: "order-abc" },
          { uid: "user-1" }
        )
      );
  ...
  describe("Orange Money API call", () => {
    it("calls the Orange Money API with correct payload", async () => {
      ...
      await capturedHandler(
        makeRequest({ amount: 5000, phoneNumber: "70123456" }, { uid: "user-1" })
      );

      expect(mockFetch).toHaveBeenCalledWith(
        "https://api.orangemoney.ml/payment",
        expect.objectContaining({
          method: "POST",
          headers: {
  ...
  describe("cart snapshot saving", () => {
    ...
    it("does not save cart snapshot when cartItems is empty", async () => {
      ...
      await capturedHandler(
        makeRequest(
          { amount: 2000, phoneNumber: "70123456", cartItems: [] },
          { uid: "user-1" }
        )
      );
    ...
    it("uses amount as subtotal fallback when subtotal is not provided", async () => {
      ...
      await capturedHandler(
        makeRequest(
          { amount: 3000, phoneNumber: "70123456", cartItems },
          { uid: "user-1" }
        )
      );
