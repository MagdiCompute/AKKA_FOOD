...
const sendOrderStatusNotification_1 = require("./sendOrderStatusNotification");
// Helper to make mockGet resolve with a Firestore-like snapshot
function makeUserSnapshot(exists, data) {
    return {
        exists,
        data: () => data,
    };
...
describe("buildNotificationPayload", () => {
    it("returns correct payload for 'confirmed' status", () => {
        const payload = (0, sendOrderStatusNotification_1.buildNotificationPayload)("confirmed", "order-123");
    ...
    it("returns correct payload for 'preparing' status", () => {
        const payload = (0, sendOrderStatusNotification_1.buildNotificationPayload)("preparing", "order-456");
        expect(payload).toEqual({
            title: "Order update",
    ...
    it("returns correct payload for 'out_for_delivery' with etaMinutes", () => {
        const payload = (0, sendOrderStatusNotification_1.buildNotificationPayload)("out_for_delivery", "order-789", 15);
        expect(payload).toEqual({
    ...
    it("returns correct payload for 'out_for_delivery' without etaMinutes", () => {
        const payload = (0, sendOrderStatusNotification_1.buildNotificationPayload)("out_for_delivery", "order-789");
        expect(payload).toEqual({
            title: "Your order is on the way!",
            body: "ETA: ? minutes",
            data: { orderId: "order-789" },
    ...
    it("returns correct payload for 'delivered' status", () => {
        const payload = (0, sendOrderStatusNotification_1.buildNotificationPayload)("delivered", "order-abc");
        expect(payload).toEqual({
    ...
    it("returns correct payload for 'failed' status", () => {
        const payload = (0, sendOrderStatusNotification_1.buildNotificationPayload)("failed", "order-def");
        expect(payload).toEqual({
    ...
    it("returns null for unknown status", () => {
        const payload = (0, sendOrderStatusNotification_1.buildNotificationPayload)("unknown_status", "order-xyz");
        expect(payload).toBeNull();
    });
    it("returns null for 'pending' status (no notification for pending)", () => {
        const payload = (0, sendOrderStatusNotification_1.buildNotificationPayload)("pending", "order-xyz");
        expect(payload).toBeNull();
    });
});
describe("sendOrderStatusNotification", () => {
    ...
    it("skips notification when user document does not exist", async () => {
        ...
        await (0, sendOrderStatusNotification_1.sendOrderStatusNotification)("order-1", "uid-1", "confirmed");
        expect(mockSend).not.toHaveBeenCalled();
    });
    it("skips notification when user has no FCM token", async () => {
        mockGet.mockResolvedValueOnce(makeUserSnapshot(true, { preferences: { notificationsEnabled: true } }));
        await (0, sendOrderStatusNotification_1.sendOrderStatusNotification)("order-1", "uid-1", "confirmed");
        expect(mockSend).not.toHaveBeenCalled();
    });
    it("skips notification when user has notifications disabled", async () => {
        mockGet.mockResolvedValueOnce(makeUserSnapshot(true, {
            fcmToken: "token-abc",
    ...
    it("skips notification for unknown status (no payload)", async () => {
        ...
        await (0, sendOrderStatusNotification_1.sendOrderStatusNotification)("order-1", "uid-1", "pending");
        expect(mockSend).not.toHaveBeenCalled();
    });
    ...
    it("sends notification with correct payload for 'out_for_delivery' with ETA", async () => {
        ...
        mockSend.mockResolvedValueOnce("message-id");
        await (0, sendOrderStatusNotification_1.sendOrderStatusNotification)("order-2", "uid-2", "out_for_delivery", 20);
        expect(mockSend).toHaveBeenCalledTimes(1);
        const message = mockSend.mock.calls[0][0];
        expect(message.notification).toEqual({
            title: "Your order is on the way!",
    ...
    it("includes Android channel and iOS sound in the message", async () => {
        ...
        mockSend.mockResolvedValueOnce("message-id");
        await (0, sendOrderStatusNotification_1.sendOrderStatusNotification)("order-3", "uid-3", "delivered");
        const message = mockSend.mock.calls[0][0];
    ...
    it("defaults notificationsEnabled to true when preferences are not set", async () => {
        ...
        mockSend.mockResolvedValueOnce("message-id");
        await (0, sendOrderStatusNotification_1.sendOrderStatusNotification)("order-4", "uid-4", "preparing");
        expect(mockSend).toHaveBeenCalledTimes(1);
        const message = mockSend.mock.calls[0][0];
        expect(message.notification).toEqual({
            title: "Order update",
    ...
    it("does not throw when FCM send fails", async () => {
        ...
        // Should not throw
        await expect((0, sendOrderStatusNotification_1.sendOrderStatusNotification)("order-5", "uid-5", "failed")).resolves.toBeUndefined();
    });
});
//# sourceMappingURL=sendOrderStatusNotification.test.js.map