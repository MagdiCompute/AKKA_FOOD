...
let collectionPathTracker: string[] = [];
...
function createRequest(data: Record<string, unknown>, auth?: { uid: string }) {
  return { data, auth };
}
...
function setupUserMock(options: { exists?: boolean; role?: string } = {}) {
...
function setupCartSnapshotMock(options: {
  ...
  const {
    exists = true,
    items = [{ name: "Meal A", price: 1000, quantity: 2 }],
  } = options;
...
function setupCartFallbackMock(options: {
  ...
  const {
    exists = true,
    items = [{ name: "Meal B", price: 500, quantity: 1 }],
  } = options;
...
describe("createOrder", () => {
  ...
  describe("authentication", () => {
    it("throws unauthenticated error when no auth context", async () => {
      ...
      await expect((createOrder as Function)(request)).rejects.toMatchObject({
  ...
  describe("transaction validation", () => {
    ...
    it("allows admin to create order for another user's transaction", async () => {
      ...
      const result = await (createOrder as Function)(request);
  ...
  describe("idempotency", () => {
    ...
    it("logs idempotent return", async () => {
      ...
      await (createOrder as Function)(request);

      expect(mockLoggerInfo).toHaveBeenCalledWith(
        "Order already exists for transaction (idempotent return)",
  ...
  describe("order creation", () => {
    ...
    it("logs order creation with details", async () => {
      ...
      expect(mockLoggerInfo).toHaveBeenCalledWith(
        "Order created via createOrder callable",
        expect.objectContaining({
          orderId: "order-new-456",
          transactionId: "txn-123",
          uid: "user-456",
