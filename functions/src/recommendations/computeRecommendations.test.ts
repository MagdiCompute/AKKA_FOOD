...
// ── Import AFTER mocks ───────────────────────────────────────────────────────
import { computeRecommendations } from "./computeRecommendations";
import { getPopularMeals } from "./computeRecommendations";

// The export is the raw handler function due to our mock of onCall
const handler = computeRecommendations as unknown as (request: unknown) => Promise<unknown>;

// ── Test helpers ─────────────────────────────────────────────────────────────
function makeRequest(auth: { uid: string } | undefined, data = {}) {
  return { auth, data };
}

function makeCacheSnapshot(
  exists: boolean,
  data?: { mealIds: string[]; isPersonalized: boolean; computedAt: { toMillis: () => number } }
) {
  return {
    exists,
    data: () => data,
...
function makeOrderDocs(orders: Array<{ id: string; items: Array<{ mealId: string }>; completedAt?: number }>) {
  ...
  return {
    docs: orders.map((order) => ({
      ...
      data: () => ({
        ...
        uid: "test-uid",
        status: "delivered",
        completedAt: { toMillis: () => order.completedAt ?? defaultCompletedAt },
      }),
    })),
  };
}

function makeMealDocs(mealIds: string[]) {
  return {
    docs: mealIds.map((id) => ({
      id,
 ...
 */
function makeMealAvailabilityDocs(
  mealIds: string[],
...
describe("computeRecommendations", () => {
  ...
  it("computes recommendations when no cache exists", async () => {
    ...
    mockGetAll.mockResolvedValueOnce(
      makeMealAvailabilityDocs(["meal-a", "meal-b", "meal-c"])
    );
  ...
  it("returns popularity-based meals for users with < 3 orders", async () => {
    ...
    mockCollectionGet.mockResolvedValueOnce(
      makeMealDocs(["pop-1", "pop-2", "pop-3", "pop-4", "pop-5"])
    );
  ...
  it("returns fewer than 10 meals when not enough are available (cold start)", async () => {
    ...
    expect((result as { mealIds: string[] }).mealIds.length).toBe(3);
  });
  ...
  it("returns personalized meals for users with >= 3 orders", async () => {
    ...
    // getAll for availability check — all available
    mockGetAll.mockResolvedValueOnce(
      makeMealAvailabilityDocs(["m1", "m2", "m3", "m4", "m5"])
    );

    mockSet.mockResolvedValueOnce(undefined);

    const result = await handler(makeRequest({ uid: "power-user" })) as { mealIds: string[]; isPersonalized: boolean };

    expect(result.isPersonalized).toBe(true);
    expect(result.mealIds).toEqual(
      expect.arrayContaining(["m1", "m2", "m3", "m4", "m5"])
    );
  ...
  it("writes computed result to /recommendations/{uid}", async () => {
    ...
    // getAll for availability check — all available
    mockGetAll.mockResolvedValueOnce(
      makeMealAvailabilityDocs(["m1", "m2", "m3"])
    );
  ...
  it("fills with popular meals when personalized results < 3 (user has >= 3 orders)", async () => {
    ...
    mockGetAll.mockResolvedValueOnce(
      makeMealAvailabilityDocs(["m1", "m2"])
    );

    // Popular meals query for fill-up
    mockCollectionGet.mockResolvedValueOnce(
      makeMealDocs(["pop-1", "pop-2", "pop-3", "pop-4", "pop-5", "pop-6", "pop-7", "pop-8", "pop-9", "pop-10"])
    );

    mockSet.mockResolvedValueOnce(undefined);

    const result = await handler(makeRequest({ uid: "fillup-user" })) as { mealIds: string[]; isPersonalized: boolean };
  ...
  it("does NOT trigger fill-up when personalized results >= 3", async () => {
    ...
    const result = await handler(makeRequest({ uid: "no-fillup-user" })) as { mealIds: string[]; isPersonalized: boolean };
  ...
  it("fill-up does not include duplicates (meals already in personalized results)", async () => {
    ...
    mockCollectionGet.mockResolvedValueOnce(
      makeMealDocs(["m1", "m2", "pop-1", "pop-2", "pop-3", "pop-4", "pop-5", "pop-6", "pop-7", "pop-8"])
    );

    mockSet.mockResolvedValueOnce(undefined);

    const result = await handler(makeRequest({ uid: "dedup-user" })) as { mealIds: string[]; isPersonalized: boolean };
...
describe("getPopularMeals", () => {
  ...
  it("queries meals collection with isAvailable == true, ordered by popularityScore desc, limit 10", async () => {
    ...
    const result = await getPopularMeals();
