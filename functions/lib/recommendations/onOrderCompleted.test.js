...
const mockIncrement = jest.fn((val) => `INCREMENT_${val}`);
...
// ── Import AFTER mocks ───────────────────────────────────────────────────────
const onOrderCompleted_1 = require("./onOrderCompleted");
// The export is the raw handler function due to our mock of onDocumentUpdated
const handler = onOrderCompleted_1.onOrderCompletedRecommendations;
// ── Test helpers ─────────────────────────────────────────────────────────────
function makeEvent(beforeData, afterData, orderId = "order-123") {
    return {
...
describe("onOrderCompletedRecommendations", () => {
    ...
    it("returns early when status did not change to delivered", async () => {
        const event = makeEvent({ status: "confirmed", uid: "user-1", items: [{ mealId: "m1" }] }, { status: "preparing", uid: "user-1", items: [{ mealId: "m1" }] });
        await handler(event);
        expect(mockUpdate).not.toHaveBeenCalled();
        expect(mockDelete).not.toHaveBeenCalled();
    });
    it("returns early when status was already delivered (no change)", async () => {
        const event = makeEvent({ status: "delivered", uid: "user-1", items: [{ mealId: "m1" }] }, { status: "delivered", uid: "user-1", items: [{ mealId: "m1" }] });
        await handler(event);
        expect(mockUpdate).not.toHaveBeenCalled();
        expect(mockDelete).not.toHaveBeenCalled();
    });
    it("returns early when newStatus is undefined", async () => {
        const event = makeEvent({ status: "confirmed", uid: "user-1", items: [{ mealId: "m1" }] }, { uid: "user-1", items: [{ mealId: "m1" }] } // no status field
        );
        await handler(event);
        expect(mockUpdate).not.toHaveBeenCalled();
        expect(mockDelete).not.toHaveBeenCalled();
    ...
    it("increments popularityScore for each meal when order is delivered", async () => {
        const event = makeEvent({ status: "preparing", uid: "user-1", items: [{ mealId: "m1" }, { mealId: "m2" }, { mealId: "m3" }] }, { status: "delivered", uid: "user-1", items: [{ mealId: "m1" }, { mealId: "m2" }, { mealId: "m3" }] });
        await handler(event);
    ...
    it("uses FieldValue.increment(1) for atomic updates (Req 5 AC2)", async () => {
        const event = makeEvent({ status: "out_for_delivery", uid: "user-1", items: [{ mealId: "meal-abc" }] }, { status: "delivered", uid: "user-1", items: [{ mealId: "meal-abc" }] });
        await handler(event);
        expect(mockIncrement).toHaveBeenCalledWith(1);
        expect(mockUpdate).toHaveBeenCalledWith({
            popularityScore: "INCREMENT_1",
    ...
    it("deletes /recommendations/{uid} to invalidate cache", async () => {
        const event = makeEvent({ status: "out_for_delivery", uid: "user-42", items: [{ mealId: "m1" }] }, { status: "delivered", uid: "user-42", items: [{ mealId: "m1" }] });
        await handler(event);
    ...
    it("handles order with empty items array gracefully", async () => {
        const event = makeEvent({ status: "preparing", uid: "user-1", items: [] }, { status: "delivered", uid: "user-1", items: [] });
        await handler(event);
    ...
    it("handles order with no items field gracefully", async () => {
        const event = makeEvent({ status: "preparing", uid: "user-1" }, { status: "delivered", uid: "user-1" });
        await handler(event);
        // No meal updates, but cache should still be invalidated
        expect(mockUpdate).not.toHaveBeenCalled();
        expect(mockDoc).toHaveBeenCalledWith("recommendations/user-1");
    ...
    it("skips items without mealId", async () => {
        const event = makeEvent({ status: "preparing", uid: "user-1", items: [{ mealId: "m1" }, { name: "no-id" }, { mealId: "m2" }] }, { status: "delivered", uid: "user-1", items: [{ mealId: "m1" }, { name: "no-id" }, { mealId: "m2" }] });
        await handler(event);
    ...
    it("does not delete recommendations cache when uid is missing", async () => {
        const event = makeEvent({ status: "preparing", items: [{ mealId: "m1" }] }, { status: "delivered", items: [{ mealId: "m1" }] });
        await handler(event);
    ...
    it("handles transition from any status to delivered (not just specific ones)", async () => {
        const event = makeEvent({ status: "confirmed", uid: "user-1", items: [{ mealId: "m1" }] }, { status: "delivered", uid: "user-1", items: [{ mealId: "m1" }] });
        await handler(event);
        expect(mockDoc).toHaveBeenCalledWith("meals/m1");
        expect(mockUpdate).toHaveBeenCalledTimes(1);
        expect(mockDoc).toHaveBeenCalledWith("recommendations/user-1");
...
//# sourceMappingURL=onOrderCompleted.test.js.map