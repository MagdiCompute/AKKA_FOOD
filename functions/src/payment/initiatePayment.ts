  ...
  async (request) => {
    ...
    if (!request.auth) {
      throw new HttpsError(
        "unauthenticated",
        "User must be authenticated to initiate a payment."
      );
    }
    ...
    const { amount, phoneNumber, orderId, cartItems, subtotal, deliveryFee, discount, redeemedCoins } = request.data as {
      amount: number;
      phoneNumber: string;
      orderId?: string;
      cartItems?: Array<{ mealId: string; mealName: string; unitPrice: number; quantity: number }>;
      subtotal?: number;
      deliveryFee?: number;
      discount?: number;
    ...
    if (!amount || typeof amount !== "number" || amount <= 0) {
      throw new HttpsError(
        "invalid-argument",
        "amount is required and must be a positive number (XOF)."
      );
    }
    ...
    if (!Number.isInteger(amount)) {
      throw new HttpsError(
        "invalid-argument",
        "amount must be a whole number (XOF does not use decimals)."
      );
    }
    ...
    if (!/^\+?223\d{8}$|^\d{8}$/.test(sanitizedPhone)) {
      throw new HttpsError(
        "invalid-argument",
        "phoneNumber must be a valid Mali phone number."
      );
    }
    ...
    try {
      ...
      throw new HttpsError(
        "internal",
        "Payment initiation failed due to a network error. Please try again."
      );
    }
