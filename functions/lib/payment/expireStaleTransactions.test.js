...
// Import AFTER mocks
require("../payment/expireStaleTransactions");
// ── Helpers ───────────────────────────────────────────────────────────────────
...
describe("expireStaleTransactions", () => {
    ...
    it("does nothing when no stale transactions are found", async () => {
        ...
        expect(mockLoggerInfo).toHaveBeenCalledWith("No stale transactions found", expect.objectContaining({ timestamp: expect.any(String) }));
    });
    it("queries Firestore for pending transactions older than 5 minutes", async () => {
        ...
        const dateArg = (_a = mockWhere.mock.calls.find((call) => call[0] === "createdAt")) === null || _a === void 0 ? void 0 : _a[2];
    ...
    it("updates stale transactions to failed status using batch writes", async () => {
        ...
        expect(mockBatchUpdate).toHaveBeenCalledWith(staleDocs[0].ref, { status: "failed", updatedAt: "SERVER_TIMESTAMP" });
        expect(mockBatchUpdate).toHaveBeenCalledWith(staleDocs[1].ref, { status: "failed", updatedAt: "SERVER_TIMESTAMP" });
    ...
    it("logs each expired transaction for audit purposes", async () => {
        ...
        mockGet.mockResolvedValueOnce({ empty: false, docs: staleDocs });
        await capturedHandler();
        expect(mockLoggerInfo).toHaveBeenCalledWith("Transaction expired", expect.objectContaining({
            transactionId: "txn-1",
            reference: "ref-1",
            uid: "user-1",
            oldStatus: "pending",
    ...
    it("logs the total count of expired transactions", async () => {
        ...
        mockGet.mockResolvedValueOnce({ empty: false, docs: staleDocs });
        await capturedHandler();
        expect(mockLoggerInfo).toHaveBeenCalledWith("Stale transactions expired", expect.objectContaining({ count: 3, timestamp: expect.any(String) }));
    });
    it("handles multiple batches when there are more than 500 transactions", async () => {
        ...
        const staleDocs = Array.from({ length: 501 }, (_, i) => makeDoc(`txn-${i}`, { reference: `ref-${i}`, uid: `user-${i}`, status: "pending" }));
        mockGet.mockResolvedValueOnce({ empty: false, docs: staleDocs });
        await capturedHandler();
...
//# sourceMappingURL=expireStaleTransactions.test.js.map