...
describe("redeemCoins", () => {
    ...
    describe("validateRedemptionAmount", () => {
        it("returns true for 1000", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(1000)).toBe(true);
        });
        it("returns true for 2000", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(2000)).toBe(true);
        });
        it("returns true for 5000", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(5000)).toBe(true);
        });
        it("returns false for 500 (not a multiple of 1000)", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(500)).toBe(false);
        });
        it("returns false for 1500 (not a multiple of 1000)", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(1500)).toBe(false);
        });
        it("returns false for 0", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(0)).toBe(false);
        });
        it("returns false for negative values", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(-1000)).toBe(false);
        });
        it("returns false for non-integer values", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(1000.5)).toBe(false);
        });
        it("returns false for NaN", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(NaN)).toBe(false);
        });
        it("returns true for very large valid multiples of 1000", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(100000)).toBe(true);
            expect((0, redeemCoins_1.validateRedemptionAmount)(1000000)).toBe(true);
        });
        it("returns false for Infinity", () => {
            expect((0, redeemCoins_1.validateRedemptionAmount)(Infinity)).toBe(false);
        });
    });
    // ── Authentication ──────────────────────────────────────────────────────────
    ...
    describe("balance validation", () => {
        ...
        it("succeeds when balance equals redeemedCoins exactly", async () => {
            ...
            expect(mockTransactionUpdate).toHaveBeenCalledWith(expect.anything(), { coinBalance: 0 });
        });
    });
    ...
    describe("successful redemption", () => {
        it("debits coins and creates CoinTransaction atomically (Req 2 AC3, Req 5 AC2)", async () => {
            ...
            expect(mockTransactionUpdate).toHaveBeenCalledWith(expect.anything(), { coinBalance: 1000 });
        });
        ...
        it("partial redemption leaves correct remaining balance", async () => {
            ...
            const result = await handler(request);
            expect(result).toEqual({ success: true });
            expect(mockTransactionUpdate).toHaveBeenCalledWith(expect.anything(), { coinBalance: 2000 } // 5000 - 3000
            );
        });
    });
    // ── Error handling ──────────────────────────────────────────────────────────
    describe("error handling", () => {
        it("re-throws HttpsError from within the transaction", async () => {
            ...
            await expect(handler(request)).rejects.toBeInstanceOf(https_1.HttpsError);
        });
...
//# sourceMappingURL=redeemCoins.test.js.map