...
jest.mock("firebase-functions/v2/firestore", () => ({
    onDocumentUpdated: jest.fn((_path, handler) => handler),
}));
// Import AFTER mocks
const onPaymentSuccess_1 = require("./onPaymentSuccess");
// The handler is the function itself (since our mock returns the handler directly)
const handler = onPaymentSuccess_1.onPaymentSuccess;
// ── Tests ─────────────────────────────────────────────────────────────────────
describe("onPaymentSuccess", () => {
    ...
    describe("calculateCoins", () => {
        it("returns 5% of amount rounded down", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(2000)).toBe(100);
        });
        it("rounds down fractional coins", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(1999)).toBe(99); // 1999 * 0.05 = 99.95 → 99
        });
        it("returns 0 for amounts less than 20", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(19)).toBe(0); // 19 * 0.05 = 0.95 → 0
        });
        it("returns 0 for zero amount", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(0)).toBe(0);
        });
        it("handles large amounts correctly", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(100000)).toBe(5000);
        });
        it("returns 1 for minimum amount that earns a coin (20 XOF)", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(20)).toBe(1); // 20 * 0.05 = 1.0 → 1
        });
        ...
        it("handles very large amounts without overflow", () => {
            ...
            expect((0, onPaymentSuccess_1.calculateCoins)(10000000)).toBe(500000);
        });
        it("returns correct coins for amount = 21 (just above boundary)", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(21)).toBe(1); // 21 * 0.05 = 1.05 → 1
        });
        it("returns correct coins for amount = 39 (just below next coin)", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(39)).toBe(1); // 39 * 0.05 = 1.95 → 1
        });
        it("returns correct coins for amount = 40 (exactly 2 coins)", () => {
            expect((0, onPaymentSuccess_1.calculateCoins)(40)).toBe(2); // 40 * 0.05 = 2.0 → 2
        });
    });
    // ── Trigger logic ───────────────────────────────────────────────────────────
    describe("trigger handler", () => {
        function makeEvent(beforeStatus, afterStatus, afterData = {}) {
            return {
                data: {
                    before: { data: () => ({ status: beforeStatus }) },
                    after: {
                        data: () => (Object.assign({ status: afterStatus, uid: "user-123", amount: 2000, orderId: "order-abc" }, afterData)),
                    },
                },
        ...
        it("credits coins via atomic transaction when all checks pass (Req 5 AC2)", async () => {
            ...
            expect(mockTransactionSet).toHaveBeenCalledWith(mockDeterministicDocRef, expect.objectContaining({
            ...
            expect(mockTransactionUpdate).toHaveBeenCalledWith(expect.anything(), { coinBalance: 600 } // 500 + 100
            );
        });
        ...
        it("initializes coinBalance to 0 if user has no existing balance", async () => {
            ...
            const event = makeEvent("pending", "success", { amount: 2000 });
            await handler(event);
            expect(mockTransactionUpdate).toHaveBeenCalledWith(expect.anything(), { coinBalance: 100 } // 0 + 100
            );
        });
...
//# sourceMappingURL=onPaymentSuccess.test.js.map