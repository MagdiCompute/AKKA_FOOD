...
describe("adminUpdateOrderStatus", () => {
  ...
  it("throws invalid-argument when status is out_for_delivery and etaMinutes is missing", async () => {
    mockGet.mockResolvedValueOnce(
      makeOrderSnapshot(true, { status: "ready_for_pickup", uid: "user-1" })
    );
    await expect(
      capturedHandler(
  ...
  it.each(invalidTransitions)(
    "throws failed-precondition for invalid transition %s → %s",
    async (from, to) => {
  ...
  const validTransitions: Array<[string, string, number?]> = [
    ["pending", "confirmed"],
    ["pending", "cancelled"],
    ["confirmed", "preparing"],
    ["confirmed", "cancelled"],
  ...
  it.each(validTransitions)(
    ...
    async (from, to, eta?) => {
      ...
      if (eta !== undefined) requestData["etaMinutes"] = eta;
  ...
  it("sends etaMinutes in the notification for out_for_delivery", async () => {
    mockGet.mockResolvedValueOnce(
      makeOrderSnapshot(true, { status: "ready_for_pickup", uid: "customer-uid" })
    );
